...
class BankEMIThinClientApproach(
    var context: Context, var cardProcessedDataModal: CardProcessedDataModal,
    var callback: (CardProcessedDataModal) -> Unit
) {
    ...
    private fun fetchBankEMIDetailsFromHost() {
        val bankEMIISOPacket = runBlocking(Dispatchers.IO) { createBankEMIPacket() }
    }
    //endregion

    //region=========================BankEMI ISO Request Packet===============================
    private fun createBankEMIPacket(): IsoDataWriter = IsoDataWriter().apply {
